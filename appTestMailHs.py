...
def send_email(subject, body, to_email, attachment=None):
    ...
    smtp_port = 587
    sender_email = "jguimfackjeuna@studenten.hs-bremerhaven.de"
    sender_password = "Jeunaj3"
    user_id = "jguimfackjeuna"

    
    try:
        if user_id is None or sender_password is None:
            sys.exit("Gib die Nutzer-ID deines Mailproviders und das Passwort im Code an.")
        print("start")
        
        # Establish SMTP connection using 'with' statement for better resource management
        ...
        print(f"Fehler: {e}")
        sys.exit("Senden der E-Mail fehlgeschlagen")
